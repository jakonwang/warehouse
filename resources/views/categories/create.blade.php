...
<div class="space-y-8" x-data="{ 
    ...
    <!-- 高级页面头部 -->
    <div class="bg-gradient-to-r from-blue-600 via-purple-600 to-blue-700 rounded-2xl shadow-xl p-8 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold mb-2">新建商品分类</h1>
                        <p class="text-blue-100 text-lg">创建新的商品分类，支持层级结构管理</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    ...
                    <p class="text-gray-600 mt-1">填写分类的基本信息和属性</p>
                </div>
                
                <form id="categoryForm" action="{{ route('categories.store') }}" method="POST" class="p-8 space-y-8">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  ...
                                  placeholder="详细描述这个分类的特点、用途和适用范围...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
                                <i class="bi bi-exclamation-circle mr-1"></i>
                                {{ $message }}
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-100">
                        ...
                        <button type="submit" 
                                ...
                                class="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                            <i class="bi bi-check-circle mr-2"></i>
                            <span x-text="isSubmitting ? '保存中...' : '保存分类'"></span>
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-4 gap-3">
                    <template x-for="icon in icons" :key="icon">
                        <button @click="selectedIcon = icon" 
                                :class="selectedIcon === icon ? 'bg-blue-100 border-blue-500' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'"
            ...
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-100">
                <h4 class="text-lg font-semibold text-gray-900 mb-4">分类统计</h4>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">总分类数</span>
                        <span class="text-lg font-bold text-purple-600">{{ $categories->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">顶级分类</span>
                        <span class="text-lg font-bold text-blue-600">{{ $categories->where('parent_id', null)->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">子分类</span>
                        <span class="text-lg font-bold text-green-600">{{ $categories->where('parent_id', '!=', null)->count() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
                <h4 class="text-lg font-semibold text-gray-900 mb-4">创建提示</h4>
                <div class="space-y-3 text-sm text-gray-600">
                    <div class="flex items-start">
                        <i class="bi bi-lightbulb text-yellow-500 mr-2 mt-0.5"></i>
                        <p>分类名称建议简洁明了，便于用户理解</p>
                    </div>
                    <div class="flex items-start">
                        <i class="bi bi-lightbulb text-yellow-500 mr-2 mt-0.5"></i>
                        <p>合理设置排序权重，重要分类可以设置较小的数值</p>
                    </div>
                    <div class="flex items-start">
                        <i class="bi bi-lightbulb text-yellow-500 mr-2 mt-0.5"></i>
                        <p>父级分类可以创建层级结构，便于商品管理</p>
                    </div>
                </div>
            </div>
        </div>
