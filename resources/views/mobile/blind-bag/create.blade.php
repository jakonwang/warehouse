...
@section('title', __('mobile.blind_bag.title'))

@section('content')
<div class="container mx-auto px-4 py-6 space-y-6 pb-40">
    <!-- {{ __('mobile.blind_bag.top_navigation') }} -->
    <div class="card p-6">
        <div class="flex items-center justify-between mb-2">
            <a href="{{ route('mobile.sales.index') }}" class="text-gray-600">
                <i class="bi bi-arrow-left text-xl"></i>
            </a>
            <h1 class="text-xl font-semibold text-gray-900">🎲 <x-lang key="mobile.sales.blind_bag_details.title"/></h1>
            <div class="w-6"></div>
        </div>
        <p class="text-gray-600"><x-lang key="mobile.sales.blind_bag_details.subtitle"/></p>
    </div>
    ...
    <form action="{{ route('mobile.blind-bag.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <!-- {{ __('mobile.blind_bag.step1_title') }} -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">🛍�?<x-lang key="mobile.sales.blind_bag_details.step1_title"/></h2>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600"><x-lang key="mobile.sales.blind_bag_details.no_products"/></p>
                    <p class="text-sm text-gray-500 mt-2"><x-lang key="mobile.sales.blind_bag_details.create_in_backend"/></p>
                </div>
            @endif
        </div>

        <!-- {{ __('mobile.blind_bag.step2_title') }} -->
        <div id="delivery-content" class="card p-6" style="display: none;">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">📦 <x-lang key="mobile.sales.blind_bag_details.step2_title"/></h2>
            <p class="text-sm text-gray-600 mb-4"><x-lang key="mobile.sales.blind_bag_details.step2_desc"/></p>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <p class="text-sm text-gray-500"><x-lang key="mobile.sales.blind_bag_details.cost"/>: ¥{{ $product->cost_price }} | <x-lang key="mobile.sales.blind_bag_details.stock"/>: <span class="stock-display" data-product="{{ $product->id }}">{{ $product->stock }}</span></p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-center space-x-3">
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-xs text-gray-500 mt-1"><x-lang key="mobile.sales.blind_bag_details.delivery_quantity"/></p>
                            </div>
                        ...
                        <!-- {{ __('mobile.blind_bag.subtotal_display') }} -->
                        <div class="mt-3 pt-3 border-t border-gray-200 text-center">
                            <span class="text-sm text-gray-600"><x-lang key="mobile.sales.blind_bag_details.cost_subtotal"/>: </span>
            ...
            @else
                <div class="text-center py-8">
                    <div class="text-4xl mb-4">📭</div>
                    <p class="text-gray-600"><x-lang key="mobile.sales.blind_bag_details.no_delivery_products"/></p>
                    <p class="text-sm text-gray-500 mt-2"><x-lang key="mobile.sales.blind_bag_details.all_out_of_stock"/></p>
                </div>
            @endif
        </div>

        <!-- {{ __('mobile.blind_bag.step3_title') }} -->
        <div id="profit-calculation" class="card p-6" style="display: none;">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">💰 <x-lang key="mobile.sales.blind_bag_details.step3_title"/></h2>
            
            <!-- {{ __('mobile.blind_bag.delivery_summary') }} -->
            <div class="bg-blue-50 rounded-lg p-4 mb-4 border border-blue-200">
                <h3 class="text-sm font-semibold text-blue-900 mb-2"><x-lang key="mobile.sales.blind_bag_details.delivery_content"/>�?/h3>
                <div id="delivery-summary" class="text-sm text-blue-800">
                    <x-lang key="mobile.sales.blind_bag_details.please_select_delivery"/>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <p class="text-xs text-gray-500"><x-lang key="mobile.sales.blind_bag_details.sale_revenue"/></p>
                </div>
                <div class="text-center p-3 bg-red-50 rounded-lg border border-red-200">
                    <div id="total_delivery_cost" class="text-lg font-bold text-red-600">¥0.00</div>
                    <p class="text-xs text-gray-500"><x-lang key="mobile.sales.blind_bag_details.delivery_cost"/></p>
                </div>
                <div class="text-center p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <div id="net_profit" class="text-lg font-bold text-purple-600">¥0.00</div>
                    <p class="text-xs text-gray-500"><x-lang key="mobile.sales.blind_bag_details.net_profit"/></p>
                </div>
            </div>
            
            <div class="mt-4 text-center">
                <span class="text-sm text-gray-600"><x-lang key="mobile.sales.blind_bag_details.profit_margin"/>: </span>
                <span id="profit_margin" class="text-lg font-bold text-orange-600">0%</span>
            </div>
        </div>

        <!-- {{ __('mobile.blind_bag.customer_info') }} -->
        <div id="customer-info" class="card p-6" style="display: none;">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">👤 <x-lang key="mobile.sales.blind_bag_details.customer_info"/></h2>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-2"><x-lang key="mobile.sales.blind_bag_details.customer_name"/></label>
                    <input type="text" name="customer_name" value="{{ old('customer_name') }}" 
                           class="form-input w-full px-3 py-2 rounded-lg border" 
                           placeholder="<x-lang key="mobile.sales.blind_bag_details.customer_name_placeholder"/>">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2"><x-lang key="mobile.blind_bag.customer_phone"/></label>
                    <input type="tel" name="customer_phone" value="{{ old('customer_phone') }}" 
                           class="form-input w-full px-3 py-2 rounded-lg border" 
                           placeholder="<x-lang key="mobile.blind_bag.customer_phone_placeholder"/>">
                </div>
            </div>
        </div>

        <!-- {{ __('mobile.blind_bag.sales_photo') }} -->
        <div id="photo-section" class="card p-6" style="display: none;">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">📷 <x-lang key="mobile.blind_bag.sale_photo"/></h2>
            <div class="space-y-4">
                ...
                <div id="photo-preview" class="hidden relative">
                    <img src="" alt="{{ __('mobile.blind_bag.preview_image') }}" class="w-full h-48 object-cover rounded-lg">
                ...
                <label for="photo" id="photo-upload-area" class="block w-full h-32 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-gray-400 transition-colors">
                    ...
                    <span class="text-sm text-gray-600"><x-lang key="mobile.blind_bag.click_to_photo"/></span>
                </label>
            </div>
        </div>

        <!-- {{ __('mobile.blind_bag.submit_button') }} -->
        <div id="submit-section" class="card p-6" style="display: none;">
            <button type="submit" id="submit-btn" disabled class="btn-primary w-full py-4 text-white font-semibold rounded-lg shadow-lg opacity-50 cursor-not-allowed">
                ...
                <x-lang key="mobile.blind_bag.submit_button"/>
            </button>
                            <p class="text-xs text-gray-500 text-center mt-2"><x-lang key="mobile.blind_bag.submit_hint"/></p>
        </div>

        <!-- {{ __('mobile.blind_bag.hidden_fields') }} -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.blind-bag-radio').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            // {{ __('mobile.blind_bag.show_next_steps') }}
    ...
    // {{ __('mobile.blind_bag.delivery_quantity_buttons') }}
