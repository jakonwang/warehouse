...
@section('title', __('messages.mobile.sales.create_title'))
...
<div class="container mx-auto px-4 py-6 space-y-6 pb-4">
    ...
    <div class="card p-6">
        <div class="flex items-center justify-between mb-2">
            ...
            <h1 class="text-xl font-semibold text-gray-900"><x-lang key="messages.mobile.sales.create_title"/></h1>
            <div class="w-6"></div>
        </div>
        <p class="text-gray-600"><x-lang key="messages.mobile.sales.create_subtitle"/></p>
    </div>
    ...
    <form action="{{ route('mobile.sales.store') }}" method="POST" class="space-y-6" x-data="saleForm()">
        ...
        <!-- 仓库选择 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">🏪 <x-lang key="messages.mobile.sales.select_store"/></h2>
            <div class="space-y-4">
        ...
        <!-- 客户信息 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">👤 <x-lang key="messages.mobile.sales.customer_info"/></h2>
            <div class="space-y-4">
                <div>
                    <label class="form-label block text-sm font-medium mb-2"><x-lang key="messages.mobile.sales.customer_name"/></label>
                    <input type="text" name="customer_name" class="form-input w-full px-3 py-2 rounded-lg border" placeholder="<x-lang key="messages.mobile.sales.customer_name_placeholder"/>">
                </div>
                <div>
                    <label class="form-label block text-sm font-medium mb-2"><x-lang key="messages.mobile.sales.customer_phone"/></label>
                    <input type="tel" name="customer_phone" class="form-input w-full px-3 py-2 rounded-lg border" placeholder="<x-lang key="messages.mobile.sales.customer_phone_placeholder"/>">
                </div>
                <div>
                    <label class="form-label block text-sm font-medium mb-2"><x-lang key="messages.mobile.sales.sales_certificate"/></label>
                    <input type="file" name="image" accept="image/*" capture="environment" class="form-input w-full px-3 py-2 rounded-lg border">
                    <p class="mt-1 text-xs text-gray-500"><x-lang key="messages.mobile.sales.certificate_desc"/></p>
                </div>
            </div>
        </div>
        ...
        <!-- 销售模式选择 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">📋 <x-lang key="messages.mobile.sales.sales_mode"/></h2>
            <div class="space-y-4">
                <div class="flex space-x-4">
                    <label class="flex items-center">
                        <input type="radio" name="sales_mode" value="standard" x-model="salesMode" class="mr-2">
                        <span class="text-sm"><x-lang key="messages.mobile.sales.standard_sales"/></span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="sales_mode" value="blind_bag" x-model="salesMode" class="mr-2">
                        <span class="text-sm"><x-lang key="messages.mobile.sales.blind_bag_sales"/></span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div x-show="salesMode === 'standard'" class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">🛍️ <x-lang key="messages.mobile.sales.standard_products"/></h2>
            <div class="space-y-4">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            ...
                            <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.price"/>: ¥{{ $product->price }}@if(auth()->user()->canViewProfitAndCost()) | <x-lang key="messages.mobile.sales.cost"/>: ¥{{ $product->cost_price }}@endif</p>
                        </div>
                        <span class="text-sm text-gray-500"><x-lang key="messages.mobile.sales.stock"/>: {{ $product->getStockQuantity() }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-center space-x-3">
                        ...
                        <div class="text-center">
                            <input type="number" 
                                   ...
                                   min="0" step="1">
                            <p class="text-xs text-gray-500 mt-1"><x-lang key="messages.mobile.sales.quantity"/></p>
                        </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-200 text-center">
                        <span class="text-sm text-gray-600"><x-lang key="messages.mobile.sales.subtotal"/>: </span>
        ...
        <div x-show="salesMode === 'blind_bag'" class="space-y-6">
            <!-- 盲袋商品选择 -->
            <div class="card p-6">
                <h2 class="text-lg font-semibold text-gray-900 mb-4">🎁 <x-lang key="messages.mobile.sales.blind_bag_products"/></h2>
                <div class="space-y-4">
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <span class="text-lg font-bold text-gray-900">{{ $product->name }}</span>
                                <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.price"/>: ¥{{ $product->price }}</p>
                            </div>
                        </div>
            ...
            <!-- 盲袋发货内容 -->
            <div class="card p-6">
                <h2 class="text-lg font-semibold text-gray-900 mb-4">📦 <x-lang key="messages.mobile.sales.delivery_content_title"/></h2>
                <p class="text-sm text-gray-600 mb-4"><x-lang key="messages.mobile.sales.delivery_content_desc"/></p>
                <div class="space-y-4">
                    @foreach($standardProducts as $product)
                    <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <span class="text-lg font-bold text-gray-900">{{ $product->name }}</span>
                                <p class="text-xs text-gray-500">@if(auth()->user()->canViewProfitAndCost())<x-lang key="messages.mobile.sales.cost"/>: ¥{{ $product->cost_price }} | @endif<x-lang key="messages.mobile.sales.stock"/>: {{ $product->getStockQuantity() }}</p>
                            </div>
                        </div>
                        ...
                        <!-- 小计显示 -->
                        <div class="mt-3 pt-3 border-t border-gray-200 text-center">
                            <span class="text-sm text-gray-600"><x-lang key="messages.mobile.sales.cost_subtotal"/>: </span>
        ...
        <!-- 销售汇总 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">📊 <x-lang key="messages.mobile.sales.sales_summary"/></h2>
            <div class="grid grid-cols-2 gap-4">
                <div class="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div class="text-lg font-bold text-blue-600">¥<span x-text="totalRevenue.toFixed(2)"></span></div>
                    <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.sales_amount"/></p>
                </div>
                ...
                <div class="text-center p-3 bg-red-50 rounded-lg border border-red-200">
                    ...
                    <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.total_cost"/></p>
                </div>
                <div class="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                    <div class="text-lg font-bold text-green-600">¥<span x-text="totalProfit.toFixed(2)"></span></div>
                    <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.total_profit"/></p>
                </div>
                <div class="text-center p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <div class="text-lg font-bold text-purple-600"><span x-text="profitRate.toFixed(1)"></span>%</div>
                    <p class="text-xs text-gray-500"><x-lang key="messages.mobile.sales.profit_rate"/></p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="card p-6">
            <button type="submit" class="btn-primary w-full py-4 text-white font-semibold rounded-lg shadow-lg">
                ...
                <x-lang key="messages.mobile.sales.save_sales_record"/>
            </button>
        </div>
    </form>
    <div class="h-24"></div>
