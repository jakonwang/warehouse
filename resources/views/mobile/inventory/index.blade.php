...
<div class="container mx-auto px-4 py-6 space-y-6 pb-4">
    <!-- 标题 -->
    <div class="card p-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">📦 <x-lang key="mobile.inventory.title"/></h1>
        <p class="text-gray-600"><x-lang key="mobile.inventory.subtitle"/></p>
    </div>

    @if($inventory->count() > 0)
        <!-- 库存概览 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">🛍�?<x-lang key="mobile.inventory.product_inventory"/></h2>
            <div class="space-y-4">
                @foreach($inventory as $item)
                    ...
                    <div class="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-lg p-4 border {{ $isOutOfStock ? 'border-red-300 bg-red-50' : ($isLowStock ? 'border-orange-300 bg-orange-50' : 'border-blue-200') }}">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @if($isOutOfStock)
                                <span class="badge-error text-xs px-2 py-1 rounded-full">
                                    <i class="bi bi-exclamation-triangle mr-1"></i><x-lang key="mobile.inventory.out_of_stock"/>
                                </span>
                            @elseif($isLowStock)
                                <span class="badge-warning text-xs px-2 py-1 rounded-full">
                                    <i class="bi bi-exclamation-triangle mr-1"></i><x-lang key="mobile.inventory.low_stock"/>
                                </span>
                            @else
                                <span class="badge-success text-xs px-2 py-1 rounded-full">
                                    <i class="bi bi-check-circle mr-1"></i><x-lang key="mobile.inventory.normal"/>
                                </span>
                            @endif
                        </div>
                        <div class="text-center">
                            ...
                            <p class="text-xs text-gray-500"><x-lang key="mobile.inventory.current_stock"/></p>
                        </div>
                        <div class="mt-3 pt-3 border-t border-gray-200">
                            <div class="flex justify-between text-xs text-gray-500">
                                <span><x-lang key="mobile.inventory.cost"/>: ¥{{ number_format($item->product->cost_price, 2) }}</span>
                                <span><x-lang key="mobile.inventory.price"/>: ¥{{ number_format($item->product->price, 2) }}</span>
                            </div>
                            @if($item->min_quantity > 0)
                            <div class="flex justify-between text-xs text-gray-500 mt-1">
                                <span><x-lang key="mobile.inventory.warning_value"/>: {{ $item->min_quantity }}</span>
                                <span><x-lang key="mobile.inventory.max_value"/>: {{ $item->max_quantity }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
        ...
        <!-- 库存统计 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">📊 <x-lang key="mobile.inventory.inventory_stats"/></h2>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-gray-600"><x-lang key="mobile.inventory.total_quantity"/></p>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4 border border-blue-200">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-gray-600"><x-lang key="mobile.inventory.total_value"/></p>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-violet-50 rounded-lg p-4 border border-purple-200">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-gray-600"><x-lang key="mobile.inventory.total_cost"/></p>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-4 border border-orange-200">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-gray-600"><x-lang key="mobile.inventory.warning_out_of_stock"/></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- 快捷操作 -->
        <div class="card p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">�?<x-lang key="mobile.inventory.quick_actions"/></h2>
            <div class="grid grid-cols-2 gap-4">
                <a href="{{ route('mobile.stock-in.index') }}" 
                   class="flex items-center justify-center p-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                    ...
                    <span class="font-semibold"><x-lang key="mobile.inventory.stock_in"/></span>
                </a>
                ...
                <a href="{{ route('mobile.sales.create') }}" 
                   class="flex items-center justify-center p-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                    ...
                    <span class="font-semibold"><x-lang key="mobile.inventory.sales_record"/></span>
                </a>
                ...
                <a href="{{ route('mobile.returns.index') }}" 
                   class="flex items-center justify-center p-4 bg-gradient-to-r from-orange-500 to-amber-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                    ...
                    <span class="font-semibold"><x-lang key="mobile.inventory.process_return"/></span>
                </a>
                ...
                <button onclick="refreshInventory()" 
                    ...
                    <span class="font-semibold"><x-lang key="mobile.inventory.refresh_data"/></span>
                </button>
            </div>
        </div>
    @else
        ...
        <div class="card p-8 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-2"><x-lang key="mobile.inventory.no_data"/></h3>
            <p class="text-gray-600 mb-6"><x-lang key="mobile.inventory.please_stock_in"/></p>
            <a href="{{ route('mobile.stock-in.index') }}" 
               class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition-colors">
                ...
                <x-lang key="mobile.inventory.stock_in_now"/>
            </a>
        </div>
    @endif
    <div class="h-24"></div>
...
function refreshInventory() {
    ...
    button.innerHTML = '<i class="bi bi-arrow-clockwise text-xl mr-2 animate-spin"></i><span class="font-semibold"><x-lang key="mobile.inventory.refreshing"/></span>';
