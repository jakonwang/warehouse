 ...
 * 公式：日均销量 × 天数 × 安全系数(1.5)
 */
function calculateSuggestedStock($avgDailySales, $days = 7) {
    $safetyFactor = 1.5; // 安全系数，考虑销售波动
    return round($avgDailySales * $days * $safetyFactor, 0);
}

// 计算7天建议备货量
$suggested_7day_stock = calculateSuggestedStock($avgDailySales, 7);

// 计算15天建议备货量  
$suggested_15day_stock = calculateSuggestedStock($avgDailySales, 15);
```

**算法说明**：
- **日均销量**：基于实际有销售记录的天数计算
- **安全系数**：1.5倍，考虑销售波动和补货周期
- **7天备货**：适合快速周转商品
- **15天备货**：适合稳定销售商品
...
- **销售频率**：进度条显示活跃天数比例
- **建议备货量**：显示7天和15天建议备货量（日均销量×天数×1.5）
- **产品详情**：点击查看详细趋势图
...
- **导出字段**：产品ID、名称、编码、总销量、总金额、订单数、平均单价、平均日销量、建议7天备货量、建议15天备货量、销售频率、趋势

### 导出示例
```csv
产品ID,产品名称,产品编码,总销量,总金额,订单数,平均单价,平均日销量,建议7天备货量,建议15天备货量,销售频率(%),趋势(%)
1,"盲袋娃娃A","BB001",156,"3120.00",45,"20.00",5.2,55,117,65.5%,+12.3%
2,"标品玩具B","ST002",98,"2940.00",32,"30.00",3.3,35,74,43.3%,-5.7%
```
